...
| \`Task\`（子代理派发）| 使用 \`subagent\` 工具（来自 pi-subagents 扩展）。支持单代理 \`subagent({ agent: "superpowers-worker", task: "..." })\`、流水线 \`subagent({ chain: [...] })\`、并行 \`subagent({ tasks: [...] })\` 等模式。详见 \`subagent-driven-development\` 技能。 |
